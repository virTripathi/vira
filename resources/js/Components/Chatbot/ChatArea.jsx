function ChatArea({ chats, defaultQuestions, addQuestion,isAnswerPending }) {
    return (
        <div id="chat-area" className="chat-area h-60 min-h-60 overflow-y-auto w-inherit max-w-96">
            ...
            <section className="ms-2 me-2 mt-2 next-possible-questions flex max-w-96 flex-wrap gap-1">
                {defaultQuestions.map((question, index) => (
                    <p key={index} onClick={() => addQuestion(question.id)} className="ms-1 mt-1 w-auto text-xs hover:cursor-pointer">
                        {question.question}
                    </p>
                ))}
            </section>
